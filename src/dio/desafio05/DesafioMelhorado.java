...
class SistemaAcionistas {
    public List<String> obterAnalisesDesempenho(String dataInicialStr, String dataFinalStr) throws ParseException {
        ...
        dataInicial = modfificarData(dataInicial, -1);
        dataFinal = modfificarData(dataFinal, +1);

        for(Analise arr : analises){
            if(arr.data.after(dataInicial) && arr.data.before(dataFinal)){
    ...
    public Date modfificarData(Date data, int num){
