...
//        LocalDate dataAtual = LocalDate.parse(dataInicialStr);
//        LocalDate dataDiaAnterior = dataAtual.minusDays(1);
//        dataInicial = df.parse(dataDiaAnterior);

        Calendar cal = Calendar.getInstance();
        cal.setTime(dataInicial);
        cal.add(Calendar.DAY_OF_MONTH, -1); // Subtrai um dia

        Calendar cal2 = Calendar.getInstance();
        cal2.setTime(dataFinal);
        cal2.add(Calendar.DAY_OF_MONTH, +1); // Subtrai um dia

        for(Analise arr : analises){
            if(arr.data.after(cal.getTime()) && arr.data.before(cal2.getTime())){
                descricoes.add(arr.descricao);
            }
        }
